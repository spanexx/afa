//! Test: crypto::seal followed by crypto::open returns the
...
fn empty_plaintext_round_trips() {
    let key = test_key();
    let (ct, nonce) = crypto::seal(b"", &key, "aad:0").expect("seal ok");
    let pt = crypto::open(&ct, &nonce, &key, "aad:0").expect("open ok");
...
fn one_byte_plaintext_round_trips() {
    let key = test_key();
    let (ct, nonce) = crypto::seal(b"x", &key, "aad:0").expect("seal ok");
    let pt = crypto::open(&ct, &nonce, &key, "aad:0").expect("open ok");
...
fn sixty_four_byte_plaintext_round_trips() {
    ...
    let (ct, nonce) = crypto::seal(&pt_in, &key, "aad:0").expect("seal ok");
    let pt_out = crypto::open(&ct, &nonce, &key, "aad:0").expect("open ok");
