//! Code Map: 3-step key wiring pattern
//! - `UnsealedHolder`: A small struct that holds the
//!   unsealed API key in memory, behind a
//!   `tokio::sync::Mutex`. Two public methods:
//!   `get_or_unseal` (the first call unseals via
//!   `security.unseal(key_name, &ctx)`; subsequent calls
//!   return the cached string) and `re_unseal_after_401`
//!   (drops the cached string and re-unseals — used when
//!   the vendor returns 401 because the key was rotated).
//!   On `Drop`, the inner `String` is overwritten with
...
//! reuses it for every later request — the guard does
//! not need to come back. But if the service says
//! "your key is bad," the specialist assumes the key
//! was rotated, drops the copy, and asks the guard
//! again. The guard hands back a fresh copy, and the
//! specialist tries the request once more. If it
//! fails again, the specialist gives up and tells the
//! workflow "your key is bad." The lockbox is
//! always locked when the specialist is not actively
//! using it, and it is wiped (zeroed) when the
//! specialist hangs up the apron at the end of the
//! shift — so the key never sits around in memory
//! after the kernel stops.
//!
//! CID Index:
//! CID:afa-plugin-llm-http-key-001 -> UnsealedHolder
//! CID:afa-plugin-llm-http-key-002 -> get_or_unseal
//! CID:afa-plugin-llm-http-key-003 -> re_unseal_after_401
//!
//! Quick lookup: rg -n "CID:afa-plugin-llm-http-key-" crates/afa-plugin-llm-http/src/key_wiring.rs
...
// API key in memory, behind a
// `tokio::sync::Mutex`. The pattern has 3 steps:
// (1) `get_or_unseal` is called the first time
// the adapter needs the key; it calls
// `security.unseal(key_name, &ctx)`, converts
// the `UnsealedSecret` to a `String` (zeroing
// the source), and caches the string;
// (2) subsequent calls return the cached string;
// (3) on HTTP 401, the adapter calls
// `re_unseal_after_401` which drops the cached
// string and re-unseals via the security engine.
// On `Drop`, the inner `String` is overwritten
...
impl UnsealedHolder {
    ...
    // zeroed on the way out, so the secret
    // never escapes its scope in a form the
    ...
    pub async fn get_or_unseal(&self, ctx: &ExecutionContext) -> Result<String, LlmErrorV1> {
        ...
        // to unseal), then unseal via the
        // security engine's `unseal` method.
        // The trait takes a `&SecretRef`
        // (the pre-sealed receipt) — not a
        ...
        if guard.is_none() {
            ...
            // the bytes into a local `Vec`,
            // build a `String` from them,
            // and zeroize the local `Vec`
            // immediately so the plaintext
            // does not linger in a
            // `Debug`-able form. The
...
impl Drop for UnsealedHolder {
    /// Wipe the cached key (if any) when the
    /// holder is dropped. The kernel does this
    /// when the adapter is dropped (at process
    /// shutdown, or when the adapter is
    /// replaced by a config reload).
    fn drop(&mut self) {
        // `try_lock` because we are in a
        // sync context (no `.await` in
        // `Drop`); if the lock is held by a
        // pending request, the request
        // will return its cloned `String`
        // and the inner `Option<String>`
        // will be dropped at the end of
...
mod tests {
    ...
    /// pre-set plaintext the first time and a
    /// *different* plaintext on every later
    /// call. Lets the test assert the holder's
    /// cache + re-unseal behavior.
    struct FakeSecurity {
    ...
    impl SecurityV1 for FakeSecurity {
        ...
        async fn unseal(
            ...
            *n += 1;
            // First call: key v1. Second
            // call: key v2 (the "rotated"
            // value).
            let bytes = if *n == 1 {
                b"sk-v1".to_vec()
            } else {
    ...
    async fn get_or_unseal_caches_on_second_call() {
        ...
        // triggers a `security.unseal` and
        // caches the result; the second
        // call returns the cached value
        // without calling `security.unseal`
        // a second time. The `FakeSecurity`
        ...
        // triggered a second `unseal` (the
        // cache is warm). The fake's
